...
def one_game_normal(p0,pIdOrder,per_player,per1,per2,per3,per4,p1,p2,p3,p4):
    ...
    while True:
        try:
            for i in range(5):
                card = np.where(env[0:56]==i)[0]
                if card.shape[0]>0:
                    print(f'Player {i} Card: {visualCard(card)}' )
                else:
...
print(n_game_normal(bot_lv0, 1, np.array([0.]), np.array([-1] + [i+1 for i in range(4)]),
                                np.array([0.]), np.array([0.]), np.array([0.]),np.array([0.]),
                                bot_lv0, bot_lv0, bot_lv0,bot_lv0)[0])